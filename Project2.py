...
def calculate_growth_rates(country_data):
    ...
    y_n_start = start_data['rgdpna'] / start_data['emp']  # Y/N
    y_n_end = end_data['rgdpna'] / end_data['emp']
    k_n_start = start_data['rkna'] / start_data['emp']    # K/N
    k_n_end = end_data['rkna'] / end_data['emp']

    g_y = (np.log(y_n_end) - np.log(y_n_start)) / years * 100
    g_k = (np.log(k_n_end) - np.log(k_n_start)) / years * 100
    g_a = (np.log(end_data['rtfpna']) - np.log(start_data['rtfpna'])) / years * 100
...
results_list = list(data.groupby('country').apply(calculate_growth_rates))
